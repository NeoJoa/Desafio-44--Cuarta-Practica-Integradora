    ...
    try {
      ...
      if (dbUser.role == "user") {
        ...
        if (user.documents.lenght != 3)
          return res.send({
            status: "error",
            message: "Necesitas subir todos los documentos",
          });
        ...
        CustomError.createError({
          ...
          cause: generateErrorInfo.dbNotChanged(),
          code: 3,
        });
      }
  ...
  postDocuments = async (req, res, next) => {
    try {
      ...
      if (req.files.identification) {
        let exists = userDocuments.findIndex(
          (element) => element == "identification"
        );
        let extension = req.files.identification[0].originalname.split(".");
        let file = {
          name: "identification",
          reference: `/public/userImages/${req.user.email}-identification.${extension[1]}`,
        };
      ...
      if (req.files.location) {
        ...
        let file = {
          name: "location",
          reference: `/public/userImages/${req.user.email}-location.${extension[1]}`,
        };
      ...
      if (req.files.accState) {
        let exists = userDocuments.findIndex(
          (element) => element == "accState"
        );
        let extension = req.files.accState[0].originalname.split(".");
        let file = {
          name: "accState",
          reference: `/public/userImages/${req.user.email}-accState.${extension[1]}`,
        };
