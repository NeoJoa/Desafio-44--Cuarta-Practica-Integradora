...
describe("Pruebas con supertest", () => {
  const user = {
    first_name: "Joaquin",
    last_name: "Aquino",
    email: "joaquinaquino999@gmail.com",
    password: "1234",
  };
  ...
  describe("Pruebas para session", () => {
    ...
    it("3. POST /api/session/login", async () => {
      const response = await requester
        .post("/api/session/login")
        .send({ email: user.email, password: user.password + "10" });
    ...
    it("5. POST /api/session/premium/:uid", async () => {
      const response = await requester
        .post(`/api/session/premium/${user.email}`)
        .set("Cookie", [coderCookie]);
      ...
      const modifiedUser = await requester
        .get("/api/session/current")
        .set("Cookie", [coderCookie]);
      expect(modifiedUser._body)
        .to.have.property("role")
        .and.not.eql(oldUser.role);
    });
  });
  ...
  describe("Pruebas para carts", () => {
    ...
    it("3. PUT /api/carts/:cid", async () => {
      // Por motivos de comodidad del test, se modifica el producto desde aca directo a la DB y no en una petición del servidor
      await productsModel.updateOne(
        { _id: pid },
        { $set: { owner: "marcosSuarez@gmail.com" } }
      );
